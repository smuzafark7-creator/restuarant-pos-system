...
export const WaiterHeader: React.FC = () => {
  ...
  return (
    <header className="h-16 bg-white text-slate-900 flex justify-between items-center w-full px-4 border-b border-slate-200 z-30 select-none shrink-0 shadow-xs font-sans">
      ...
      <div className="flex items-center gap-2.5 sm:gap-3 shrink-0">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-slate-500">
            ...
            <span>•</span>
            <span className="text-amber-700 font-semibold">{occupiedCount} Active</span>
          </div>
        </div>
      </div>
      ...
      <div className="hidden md:flex items-center gap-2 max-w-md w-full mx-4">
        <div ref={searchContainerRef} className="relative flex-1">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-7 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-900 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-emerald-500 shadow-2xs transition-colors"
            />
            {tableSearchTerm && (
              <button
                type="button"
          ...
          {isSearchDropdownOpen && matchingTables.length > 0 && (
            <div className="absolute left-0 right-0 top-full mt-1.5 bg-white border border-slate-200 rounded-xl shadow-lg p-1.5 z-50 text-xs max-h-56 overflow-y-auto">
              ...
              {matchingTables.map(tbl => (
                <button
                  ...
                  className="w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg text-left text-slate-700 hover:bg-slate-100 transition-colors"
                >
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-slate-900">T{tbl.number}</span>
                    <span className="text-[11px] text-slate-500">{tbl.name}</span>
                  </div>
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-50 border border-slate-200 text-[11px] text-slate-700 shrink-0">
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-slate-50 border border-slate-200 text-xs">
          <div className="w-6 h-6 rounded-md bg-slate-200 flex items-center justify-center text-slate-700">
            <User className="w-3.5 h-3.5" />
          </div>
          <div className="hidden sm:flex flex-col text-left">
            <span className="text-slate-900 font-semibold text-[11px] leading-tight">{waiterName}</span>
            <span className="text-emerald-700 text-[9px] font-bold leading-tight">ACTIVE SHIFT</span>
          </div>
        </div>
        ...
        <button
          type="button"
          onClick={logout}
          className="p-2 rounded-lg bg-slate-50 hover:bg-rose-50 text-slate-600 hover:text-rose-600 border border-slate-200 transition-colors cursor-pointer"
          title="Sign Out"
        >
