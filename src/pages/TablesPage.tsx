...
export const TablesPage: React.FC = () => {
  ...
  const getStatusBadge = (status: TableStatus) => {
    switch (status) {
      ...
      case 'ready':
        return <span className="px-2 py-0.5 rounded-md text-[10px] font-semibold bg-emerald-950/60 text-emerald-400 border border-emerald-800">FOOD READY</span>;
      case 'waiting':
  ...
  return (
    <div className="w-full p-2.5 sm:p-3.5 space-y-2.5 sm:space-y-3 select-none text-slate-200 font-sans">
      {/* Top Header & Filters (Dark Slate Card Surface) */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-xl border border-slate-800 bg-[#0f172a] shadow-xs w-full text-slate-200">
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 overflow-x-auto pb-0.5 md:pb-0 text-xs shrink-0">
              ...
              <button
                ...
                className={`px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors cursor-pointer ${
                  statusFilter === status
                    ...
                    : 'bg-[#080d1a] hover:bg-slate-800 text-slate-300 border border-slate-800'
                }`}
              >
                {status} ({count})
              </button>
            ...
            {['All', 'Ground Floor', 'First Floor', 'Outdoor / Terrace'].map(fl => {
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-colors cursor-pointer whitespace-nowrap flex items-center gap-1.5 ${
                    isSelected
                      ? 'bg-emerald-600 text-white shadow-xs'
                      : 'bg-[#0f172a] hover:bg-slate-800 text-slate-300 border border-slate-800 shadow-2xs'
                  }`}
                >
                  <span>{fl}</span>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            >
              ...
              <span className={`text-[10px] px-1.5 py-0.2 rounded font-bold ${
                assignedFilter === 'mine' ? 'bg-emerald-700 text-emerald-100' : 'bg-slate-800 text-slate-400'
              }`}>
            ...
            <button
              ...
              className={`px-2 py-1 rounded-lg text-xs font-semibold transition-all cursor-pointer whitespace-nowrap ${
                assignedFilter === 'all'
        ...
        {displayedTables.map(tbl => {
          ...
          return (
            <div
              ...
              className={`p-3 rounded-xl border-[1.5px] bg-[#161B26] hover:bg-[#1F2637] transition-all duration-200 ease-[cubic-bezier(0.4,0,0.2,1)] hover:-translate-y-1 cursor-pointer flex flex-col justify-between min-h-[185px] relative group select-none ${getCardBorder(
                tbl.status
              )}`}
            >
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    {tbl.assignedWaiterName && (
                      <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold truncate max-w-[70px] ${
                        tbl.assignedWaiterName === currentUser?.name || tbl.assignedWaiterId === currentUser?.id
                ...
                <div className="flex items-baseline justify-between">
                  ...
                  <span className="text-[10px] text-slate-400">{getTableFloor(tbl.number)}</span>
                </div>
                ...
                {isOccupiedOrBilling && (
                  <div className="mt-2.5 pt-2 border-t border-slate-800 text-xs space-y-1">
                    {tbl.seatedAt && (
                    ...
                    {tableKots.length > 0 && (
                      <div className="flex items-center justify-between text-[11px] text-slate-300">
                        ...
                        <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-amber-950/60 text-amber-300 border border-amber-800">
                          {tableKots.length} KOT{tableKots.length > 1 ? 's' : ''}
                        </span>
                      </div>
                    )}
                    {tableRunningAmount > 0 && (
                      <div className="text-xs font-bold pt-0.5 text-emerald-400">
              ...
              <div className="mt-3 pt-2 border-t border-slate-800">
                {tbl.status === 'cleaning' ? (
                  <button
                    ...
                    className="w-full py-1.5 rounded-lg text-xs font-semibold flex items-center justify-center gap-1 transition-colors bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 cursor-pointer"
                  >
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      type="button"
                      onClick={() => selectTableForPOS(tbl.name)}
                      className="flex-1 py-1.5 rounded-lg text-xs font-bold flex items-center justify-center gap-1 transition-colors bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer shadow-xs"
                    >
                    ...
                    <button
                      ...
                      className="px-2 py-1.5 rounded-lg text-[10px] font-semibold border bg-slate-800 hover:bg-slate-700 text-slate-300 border-slate-700 cursor-pointer"
                  ...
                  <div className="flex items-center gap-1.5 text-[10px]">
                    <button
                      ...
                      className="flex-1 py-1.5 rounded-lg font-bold text-center transition-colors border bg-slate-800 hover:bg-slate-700 text-slate-200 border-slate-700 cursor-pointer"
                    ...
                    {tableKot && (
                      <button
                        ...
                        className="py-1.5 px-2 rounded-lg font-bold text-center transition-colors border bg-slate-800 hover:bg-slate-700 text-amber-300 border-slate-700 cursor-pointer"
                    ...
                    {isWaiter ? (
                      tbl.status === 'billing' || billRequests.some(r => r.tableNumber.toLowerCase() === tbl.name.toLowerCase() && r.status === 'pending') ? (
                        <button
                          disabled
                          className="flex-1 py-1.5 rounded-lg bg-purple-950/60 text-purple-300 border border-purple-800 font-bold text-center text-[10px] cursor-not-allowed"
                        ...
                        <button
                          ...
                          className="flex-1 py-1.5 rounded-lg bg-amber-600 hover:bg-amber-700 text-white font-bold text-center transition-colors shadow-2xs text-[10px] cursor-pointer"
                      ...
                      <button
                        ...
                        className={`flex-1 py-1.5 rounded-lg text-white font-bold text-center transition-colors shadow-2xs cursor-pointer ${
      ...
      {displayedTables.length === 0 && (
        <div className="p-12 text-center text-xs rounded-xl border border-slate-800 font-sans text-slate-400 bg-[#0f172a] shadow-2xs">
