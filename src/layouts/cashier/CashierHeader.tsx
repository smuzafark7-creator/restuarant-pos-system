...
export const CashierHeader: React.FC<CashierHeaderProps> = ({ printerStatus = 'online' }) => {
  ...
  return (
    <header className="h-16 bg-[#0F172A] text-white px-3 sm:px-5 flex items-center justify-between border-b border-slate-800 z-30 select-none shrink-0 font-mono shadow-md">
      ...
      <div className="flex items-center gap-3 md:gap-4 flex-1 max-w-2xl min-w-0 mr-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div>
            ...
            <div className="text-[10px] text-emerald-400 font-medium">Counter Billing #01</div>
          </div>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-sm transition-all cursor-pointer shrink-0"
        ...
        <div className="relative flex-1 max-w-xs sm:max-w-md min-w-[140px]" ref={searchContainerRef}>
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="pl-8 pr-7 py-1.5 rounded-lg bg-slate-800/90 border border-slate-700 focus:border-emerald-500 text-xs text-white placeholder-slate-400 w-full transition-all focus:outline-none"
            />
            {billSearchQuery && (
              <button
                type="button"
          ...
          {isSearchOpen && billSearchQuery.trim().length > 0 && (
            <div className="absolute left-0 mt-1.5 w-80 sm:w-96 bg-[#1E293B] border border-slate-700 rounded-xl shadow-2xl p-2 z-50 text-xs animate-in fade-in zoom-in-95">
              <div className="px-2 py-1 flex items-center justify-between text-[10px] text-slate-400 uppercase border-b border-slate-700 mb-1.5">
                ...
                <div className="space-y-1 max-h-60 overflow-y-auto">
                  {matchingBills.map(bill => (
                    <button
                      ...
                      className="w-full text-left p-2 rounded-lg bg-slate-800/60 hover:bg-slate-800 border border-slate-700/60 hover:border-emerald-500/60 transition-all cursor-pointer"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-white text-xs">{bill.billNumber}</span>
                      ...
                      <div className="flex justify-between text-[10px] text-slate-400 mt-0.5">
                        ...
                        <span className="uppercase font-semibold">{bill.paymentMethod}</span>
                      </div>
                    </button>
                  ))}
                </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3 text-xs shrink-0">
        ...
        <div className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800/90 border border-slate-700 text-slate-300">
          <DollarSign className="w-3.5 h-3.5 text-emerald-400" />
          <span className="text-slate-400 text-[11px]">Drawer Cash:</span>
          <span className="font-bold text-emerald-300">₹{drawerCashBalance.toLocaleString('en-IN')}</span>
        </div>

        {/* Cashier Shift summary */}
        <div className="hidden xl:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800/90 border border-slate-700 text-slate-300">
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-800/80 border border-slate-700/80 text-[11px]">
          <Printer className="w-3.5 h-3.5 text-emerald-400" />
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
          <span className="text-emerald-300 font-semibold hidden sm:inline">Printer Ready</span>
        </div>
